...
public class Jump {
	...
	private static float rate = 1.48f;
	private static boolean running = false;
	private static boolean restart = true;
	private static int step = 0;
	JTextField rateField;
	JFrame view;
	static Jump j = new Jump();
	...
	public static void main(String[] args) {
		...
		if (ANDROID_HOME == null) {
			ANDROID_HOME = new File("customFunction/bin/Jump/").getAbsolutePath();
		}
	...
	private void showView() {
		view = new JFrame("微信跳一跳辅助");
		view.setSize(220, 220);
		view.setResizable(false);
		view.setLocationRelativeTo(null);
		ImageIcon imageIcon = new ImageIcon("customFunction/bin/Jump/platform-tools/jump.jpg");
		view.setIconImage(imageIcon.getImage());
		...
		rateField = new JTextField(rate + "", 3);
		...
		for (int i = height / 3; i < height * 3 / 4; i++) {
			...
			if (y2 > 0) {
				...
				for (i = i + 50; (i += 5) < height * 3 / 4;) {
					...
					if (p[0] < 240 && Math.abs(p[0] - p2[0]) > 5 && Math.abs(p[1] - p2[1]) > 5
							&& Math.abs(p[2] - p2[2]) > 5) {
						y2 += (i - y2) / 2;
						break;
					}
