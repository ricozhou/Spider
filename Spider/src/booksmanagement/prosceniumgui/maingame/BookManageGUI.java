package booksmanagement.prosceniumgui.maingame;
...
public class BookManageGUI extends JFrame implements ActionListener {
	...
	public String[] tabNames = { "图书管理", "小工具", "小游戏", "小通讯", "自定义应用", "小设置" };
	...
	public BookManageGUI(PassParameter passParameter) throws IOException {
		initMenu();
	...
	private void initJFrame() throws IOException {
		...
		jlb12 = new JLabel("_____________________________________________________________");
		...
		this.setTitle("图书管理系统 - (" + passParameter.getUserInfo().getUserName() + ")");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (key.equals(button1)) {
			// 获取参数
			BookInformation bookInformation = getBookInfo();
			if (bu.checkBookInfo(bookInformation)) {
	...
	// 获取详细信息参数
	private BookInformation getBookInfo() {
		BookInformation bookInformation = new BookInformation();
		if (jrb1.isSelected()) {
			// 获取书名
			bookInformation.setBookName(jtf1.getText().trim());
			// 获取作者
			bookInformation.setBookAuthor(jtf2.getText().trim());
			// 出版社
			bookInformation.setBookPress(jtf3.getText().trim());
			// 分类
			bookInformation.setBookClassify(jcb1.getSelectedItem().toString());
			// 出版时间
			bookInformation.setBookPublicationTime(jtf7.getText().trim());
			// 入库时间
			bookInformation.setBookBeInAddress(jtf8.getText().trim());
			// 是否已读
			bookInformation.setBookAlreadyRead(jck1.isSelected());
			// 是否想读
			bookInformation.setBookWantRead(jck1.isSelected());
			// 入库地点
			bookInformation.setBookBeInAddress(jtf4.getText().trim());
			// 阅读指数
			bookInformation.setBookReadIndex(jtf9.getText().trim());
		} else {
			// 获取书名
			bookInformation.setBookName(jtf5.getText().trim());
			// 获取作者
			bookInformation.setBookAuthor(jtf6.getText().trim());
		}
