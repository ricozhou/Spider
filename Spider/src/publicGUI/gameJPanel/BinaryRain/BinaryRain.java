package publicGUI.gameJPanel.BinaryRain;
...
public final class BinaryRain extends JDialog {
	...
	public BinaryRain() {
		try {
			...
			JOptionPane.showMessageDialog(null, "初始化失败！请重试！", "BinaryRain", JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}
	}
	...
	private void initProperties() throws Exception {
		...
		File f = new File("gameResource/BinaryRain/BinaryRainProperties.properties");
	...
	private void init() {
		...
		// 标题
		this.setTitle("Binary Rain");
		BufferedImage cursor = new BufferedImage(16, 16, BufferedImage.TYPE_INT_ARGB_PRE);
		this.setCursor(Toolkit.getDefaultToolkit().createCustomCursor(cursor, new Point(8, 8), "Disable Cursor"));
		this.setSize(size);
...

	{
		...
		// try {
		// rainFont = Font.createFont(Font.TRUETYPE_FONT,
		// getClass().getResourceAsStream("RainFont.ttf")).deriveFont(Font.BOLD, 15.0f);
	...
	private final class RainPanel extends JPanel {
		...
		private final Color[] COLORS = new Color[] { new Color(255, 0, 0), new Color(255, 165, 0),
				new Color(255, 255, 0), new Color(0, 255, 0), new Color(0, 127, 0), new Color(0, 127, 255),
				new Color(139, 0, 255), };
	}
